...
const Contact: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto reveal-animation space-y-12 pb-24 px-4">
      {isSubmitted && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-sm p-8 rounded-xl border-2 shadow-2xl ${currentColors.card} ${currentColors.border}`}>
            ...
            <button onClick={() => setIsSubmitted(false)} className="mt-8 w-full py-3 rounded-lg bts-gradient text-white font-bold">Fechar</button>
          </div>
        </div>
      )}
      ...
      <div className={`p-8 md:p-12 rounded-xl border-2 ${currentColors.card} ${currentColors.border} shadow-xl`}>
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-3">
            <label className={`text-sm font-bold uppercase ${currentColors.text}`}>Seu nome</label>
            <input type="text" name="nome" required placeholder="Digite seu nome" className={`w-full p-5 rounded-lg border-2 outline-none bg-gray-50/50 dark:bg-black/10 ${currentColors.border} ${currentColors.text}`} />
          </div>
          <div className="space-y-3">
            <label className={`text-sm font-bold uppercase ${currentColors.text}`}>Seu email</label>
            <input type="email" name="email" required placeholder="seu@email.com" className={`w-full p-5 rounded-lg border-2 outline-none bg-gray-50/50 dark:bg-black/10 ${currentColors.border} ${currentColors.text}`} />
          </div>
          <div className="space-y-3">
            <label className={`text-sm font-bold uppercase ${currentColors.text}`}>Sua mensagem</label>
            <textarea name="mensagem" required placeholder="Escreva sua mensagem aqui..." className={`w-full p-6 rounded-xl border-2 outline-none min-h-[180px] resize-none bg-gray-50/50 dark:bg-black/10 ${currentColors.border} ${currentColors.text}`} />
          </div>
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-5 rounded-xl bts-gradient text-white font-bold text-lg shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
          >
